...
namespace CrawlerAPI.CrawlingFunctionsBBC
{
    public class BusinessCrawler
    {
        public async Task<List<News>> StartCrawlerAsync()
        {
            ...
            var url = "https://www.bbc.co.uk/news/business";
            var httpClient = new HttpClient();
            var html = await httpClient.GetStringAsync(url);
            var htmlDocument = new HtmlDocument();
            htmlDocument.LoadHtml(html);
            var divs = htmlDocument.DocumentNode.Descendants("div").Where(node => node.GetAttributeValue("class", "").Equals("gel-layout__item gs-u-pb+@m gel-1/1@m gel-1/1@l gel-1/3@xl gel-1/3@xxl nw-o-keyline nw-o-no-keyline@l") || node.GetAttributeValue("class", "").Equals("gel-layout__item gel-1/1@m gel-1/3@xl gel-1/1@l gel-1/3@xxl nw-o-keyline nw-o-no-keyline@m")).ToList();
